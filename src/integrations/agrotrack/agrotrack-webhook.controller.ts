...
export class AgroTrackWebhookController {
  ...
  @ApiOperation({ summary: 'Receive an AgroTrack status webhook (signed, not JWT-authenticated)' })
  @ApiResponse({ status: 200, description: 'Webhook processed or already seen (idempotent)' })
