...
import {
  ...
  OrderState,
  AgroTrackStatus,
} from './entities/buy-request.entity';
...
import {
  ...
  isValidBase64SizeGeneric,
  PO_SUPPORTED_MIME_TYPES,
  SUPPORTED_MIME_TYPES,
} from 'src/common/utils/base64.util';
...
} from 'src/product-inventories/entities/product-inventory.entity';
...
export class BuyRequestsService {
    ...
    try {
      if (dto.isGeneral) {
        if (dto.sellerId)
          throw new BadRequestException(
            `General buy request doesn't require a sellerId to be provided at initiation`,
          );
        if (dto.productId)
          throw new BadRequestException(
            `General buy request doesn't require a productId to be provided at initiation`,
          );
        if (purchaseOrderDoc)
          throw new BadRequestException(
            `General buy request creation doesn't require purchaseOrderDoc upload`,
          );
      } else {
        if (!dto.sellerId)
          throw new BadRequestException(
            `buyRequest requires a sellerId to be provided at initiation`,
          );
        if (!dto.productId)
          throw new BadRequestException(
            `buyRequest requires a productId to be provided at initiation`,
          );
        if (!purchaseOrderDoc)
          throw new BadRequestException(
            `buyRequest requires purchaseOrderDoc upload`,
          );
      }
      ...
      if (purchaseOrderDoc) {
        ...
        if (!detected || !PO_SUPPORTED_MIME_TYPES.includes(detected)) {
          throw new BadRequestException(
            'PurchaseOrderDoc must be DOC or PDF format.',
          );
        }
        ...
        if (!isValidBase64SizeGeneric(purchaseOrderDoc, docSize)) {
          throw new BadRequestException(
            'PurchaseOrderDoc must be 1MB or less.',
          );
        }
      }
      ...
      if (dto.productId && product && product.cropType.id !== dto.cropId) {
        throw new BadRequestException(
          `Seller's product crop type: ${product.cropType.name} should match the crop type required on BuyRequest`,
        );
      }
      ...
      const seller = dto.sellerId
        ? await this.usersService.findUserEntity(dto.sellerId)
        : null;
      ...
      // Upload doc
      if (purchaseOrderDoc) {
        const uploadedDoc = await this.fileService.uploadFile(
          purchaseOrderDoc,
          `PurchaseOrderDoc`,
          savedBuyRequest,
        );
      ...
      if (dto.isGeneral) {
        ...
        for (const farmer of farmers) {
          await this.notificationsService.createNotification({
            ...
            message: `New general buyer's request REQ-${savedBuyRequest.requestNumber} is now available on the system. Processor is asking for a ${crop.name.toUpperCase()} product.`,
            relatedEntityType: RelatedEntityType.BuyRequest,
            relatedEntityId: savedBuyRequest.id,
          });
        }
    ...
    try {
      ...
      if (buyRequest.buyer.id !== currentUser.id) {
        throw new ForbiddenException(
          'You are not authorized to update this buy request',
        );
      }
  ...
  async updateStatus1(
    ...
    try {
      ...
      if (
        ...
        !editableStates.includes(buyRequest.orderState)
      ) {
        throw new BadRequestException('BuyRequest cannot be updated');
      }
      ...
      if (buyRequest.isGeneral) {
        ...
        throw new BadRequestException(
          'BuyRequest is general, contact the buyer',
        );
      ...
      // Case: Directed buyrequest
        ...
        if (!buyRequest.seller || buyRequest.seller.id !== currentUser.id) {
          throw new ForbiddenException(
            'This farmer is not authorized to update this request',
          );
        }
        ...
        if (
          ...
          throw new BadRequestException(
            'Invalid status update for directed buy request',
          );
        }
      ...
      // Notification to Buyer
      if (dto.status !== BuyRequestStatus.ACCEPTED) {
        const title = `Buy Request ${savedBuyRequest.status.toLowerCase()}: REQ-${savedBuyRequest.requestNumber}`;
        const message = `Your buy request REQ-${buyRequest.requestNumber} has been ${savedBuyRequest.status.toLowerCase()} by ${buyRequest.seller.firstName.toLowerCase()}.`;
  ...
  async updateOrderState(
    ...
    try {
      ...
      if (!isAdmin && !isBuyer && !isSeller) {
        throw new ForbiddenException(
          'You are not authorized to update order state for this request',
        );
      }
      ...
      if (dto.orderState === OrderState.IN_TRANSIT && !isAdmin && !isSeller) {
        throw new ForbiddenException(
          'Only admin or seller can set order state to in_transit',
        );
      }

      if (dto.orderState === OrderState.AWAITING_SHIPPING) {
        throw new ForbiddenException(
          'Order cannot be manually reverted to awaiting_shipping',
        );
      }

      if (dto.orderState === OrderState.DELIVERED && !isAdmin && !isBuyer) {
        throw new ForbiddenException(
          'Only buyer or admin can mark as DELIVERED',
        );
      }
      ...
      const updatedBuyRequest =
        await this.buyRequestsRepository.save(buyRequest);

      // Notification to Seller
      if (buyRequest.seller != null) {
        const title = `Order Delivered: REQ-${buyRequest.requestNumber}`;
        const message = `Your buy request REQ-${buyRequest.requestNumber} has been delivered to ${buyRequest.buyer.companyName?.toUpperCase()}.`;
  ...
  async updateTracking(
    ...
    try {
      ...
      handleServiceError(
        error,
        'An error occurred while linking AgroTrack tracking',
      );
    }
  }
   ...
   */
  async arrangeTransitViaAgroTrack(
    id: string,
    dto: ArrangeTransitDto,
    currentUser: User,
  ): Promise<any> {
    try {
      const buyRequest = await this.buyRequestsRepository.findOne({
      ...
      if (!isAdmin && !isSeller) {
        throw new ForbiddenException(
          'Only the farmer on this request can arrange transit',
        );
      }

      if (!buyRequest.seller) {
        throw new BadRequestException(
          'This buy request has no farmer/seller assigned yet',
        );
      }
      ...
      let result: AgroTrackOrderResult;
      try {
        ...
        if (err instanceof AgroTrackSenderUnresolvedError) {
          this.logger.warn(
            `AgroTrack sender unresolved for buyRequest ${buyRequest.id}: ${err.message}`,
          );
          return {
            statusCode: 200,
            message:
              'Could not arrange automatically — use the manual Arrange Transit flow',
            data: instanceToPlain(buyRequest),
      ...
      handleServiceError(
        error,
        'An error occurred while arranging transit with AgroTrack',
      );
    }
  }
  ...
  async cancelAgroTrackShipment(id: string, currentUser: User): Promise<any> {
    try {
      ...
      if (!isAdmin && !isSeller) {
        throw new ForbiddenException(
          'Only the farmer on this request can cancel its AgroTrack shipment',
        );
      }

      if (!buyRequest.agroTrackTrackingNumber) {
        throw new BadRequestException(
          'No AgroTrack shipment is linked to this request',
        );
      }
      ...
      handleServiceError(
        error,
        'An error occurred while cancelling the AgroTrack shipment',
      );
    }
  }
  ...
  async getAllBuyRequests(query: GetAllBuyRequestsQueryDto): Promise<any> {
    try {
      ...
      handleServiceError(
        error,
        'An error occurred while fetching buy requests',
      );
    }
  }
  ...
  async findGeneralRequests(): Promise<any> {
    try {
      ...
      handleServiceError(
        error,
        'An error occurred, while fetching general BuyRequest',
      );
    }
  }
  ...
  async findMyBuyRequests(
    ...
    try {
      ...
      if (currentUser.role === UserRole.FARMER) {
        ...
        throw new ForbiddenException(
          'Only farmers and processors can view buy requests',
        );
      }
      ...
      if (status && !allowedStatuses.includes(status)) {
        throw new ForbiddenException(
          `You are not allowed to view requests with status ${status}`,
        );
      }
      ...
      const qb = this.buyRequestsRepository
        ...
        .leftJoinAndSelect('buyRequest.cropType', 'cropType')
        .leftJoinAndSelect(
          'buyRequest.qualityStandardType',
          'qualityStandardType',
        )
        .leftJoinAndSelect('buyRequest.buyer', 'buyer')
        .leftJoinAndSelect('buyRequest.seller', 'seller')
      ...
      if (status) {
        ...
        qb.andWhere('buyRequest.status IN (:...statuses)', {
          statuses: allowedStatuses,
        });
      }

      if (currentUser.role === UserRole.FARMER) {
        qb.andWhere('buyRequest.sellerId = :userId', {
      ...
      qb.orderBy('buyRequest.createdAt', 'DESC').skip(skip).take(pageSize);
  ...
  async findOngoingBuyRequestOrders(query: OngoingBuyRequestOrdersQueryDto) {
    try {
      const { search, state, pageNumber = 1, pageSize = 20 } = query;
      ...
      if (state) {
        ...
        qb.andWhere('buyRequest.orderState IN (:...states)', {
          states: [
      ...
      handleServiceError(
        error,
        'An error occurred while fetching ongoing buy requests',
      );
    }
  }
  ...
  async findUserBuyRequests(userId: string): Promise<any> {
    try {
      ...
      const query = this.buyRequestsRepository
        .createQueryBuilder('buyRequest')
        .leftJoinAndSelect('buyRequest.cropType', 'cropType')
        .leftJoinAndSelect(
          'buyRequest.qualityStandardType',
      ...
      if (user.role === UserRole.FARMER) {
        ...
        query
          .andWhere('seller.id = :userId', { userId })
        ...
        throw new BadRequestException(
          'User role not authorized to view buy requests',
        );
      }
      ...
      handleServiceError(
        error,
        'An error occurred while fetching user buy requests',
      );
    }
  }
  ...
  async directBuyRequest(
    ...
    try {
      ...
      if (buyRequest.buyer.id !== buyer.id) {
        throw new ForbiddenException(
          'You are not allowed to direct this buy request',
        );
      }
      ...
      if (!buyRequest.isGeneral) {
        throw new BadRequestException(
          'Only general buy requests can be directed',
        );
      }
      ...
      if (buyRequest.seller) {
        throw new BadRequestException(
          'Buy request has already been directed to a seller',
        );
      }
      ...
      handleServiceError(
        error,
        'An error occurred, failed to direct buy request',
      );
    }
  }
  ...
  async deleteRequest(id: string, currentUser: User): Promise<any> {
    try {
      ...
      if (!buyRequest || buyRequest.isDeleted)
        throw new NotFoundException('BuyRequest not found');
      ...
      if (buyRequest.status !== BuyRequestStatus.PENDING)
        throw new ForbiddenException('BuyRequest cannot be deleted');
  ...
  async updateStatus(
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        if (buyRequest.status === BuyRequestStatus.CANCELLED)
          throw new BadRequestException('BuyRequest cannot be updated');
        ...
        if (
          ...
          const message = `Your buy request REQ-${buyRequest.requestNumber} has been ${dto.status.toLowerCase()} by ${buyRequest.seller.firstName.toLowerCase()}.`;
